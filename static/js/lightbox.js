...
function display_image(payload, options) {
    ...
    $(".image-actions, .image-description, .download, .smart-photo-trigger").show();

    if (options.smart_photo === true) {
        var canvas = document.createElement("canvas");
        canvas.setAttribute("data-src", payload.source);

        $("#lightbox_overlay .image-preview").html(canvas).show();
        var photo = new SmartPhoto(canvas);
        photo.speed(2.3);
    } else {
        var img = new Image();
...
function display_youtube_video(payload) {
    ...
    $("#lightbox_overlay .image-preview, .image-description, .download, .smart-photo-trigger").hide();
...
exports.open = function (image, options) {
    if (!options) {
        options = {
            ...
            smart_photo: $(".smart-photo-trigger").hasClass("enabled"),
        };
    }
...
$(function () {
    ...
    $("#lightbox_overlay .image-preview").on("dblclick", "img, canvas", function (e) {
        $("#lightbox_overlay .smart-photo-trigger").click();
        e.preventDefault();
    });
});
